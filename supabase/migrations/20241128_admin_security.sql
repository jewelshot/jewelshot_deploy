...
-- Date: 2024-11-28
-- Version: 1.0
-- Description: Add admin roles and comprehensive audit logging
-- Author: Security Sprint Team
-- ============================================
...
-- Drop table if exists (for migration re-runs)
...
CREATE POLICY "Service role can insert audit logs"
  ...
  WITH CHECK (true);
  -- Note: In production, this uses service role key which bypasses RLS
...
SELECT 
  ...
  'test',
  true,
  '{"note": "Sample audit log entry"}'::jsonb
FROM users
WHERE email = 'admin@test.jewelshot.ai';
*/
...
-- WHERE table_name = 'users' AND column_name IN ('role', 'is_2fa_enabled', 'two_factor_secret');
...
-- Next step: Manually set your user as admin (see MANUAL_ADMIN_SETUP.md)
