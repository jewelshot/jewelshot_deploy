...
import { X, Download, Eye, FolderOpen } from 'lucide-react';
...
interface BatchDetailModalProps {
  ...
  onViewImage: (image: { originalUrl: string | null; generatedUrl: string; name: string }) => void;
...
export function BatchDetailModal({
  ...
  return (
    ...
    >
      <div
        className="relative w-full max-w-6xl my-8 rounded-2xl border border-white/10 bg-[rgba(10,10,10,0.95)] p-6 shadow-2xl"
            ...
            <div>
              ...
              <div className="mt-1 flex items-center gap-4 text-sm text-white/60">
                ...
                {project.failed_images > 0 && (
                  <>
                    <span>•</span>
                    <span className="text-red-400">{project.failed_images} failed</span>
                  </>
                )}
              </div>
            </div>
        ...
        {images.length > 0 ? (
          <div className="grid grid-cols-1 gap-4 md:grid-cols-2 lg:grid-cols-3">
            {images.map((image) => (
              <div
                key={image.id}
                className="group relative overflow-hidden rounded-lg border border-white/10 bg-white/[0.02] transition-all hover:border-purple-500/30"
              >
                {/* Image Preview */}
                <div className="relative aspect-square overflow-hidden bg-black/20">
                  ...
                  {/* Hover Actions */}
                  <div className="absolute inset-0 flex items-center justify-center gap-2 bg-black/70 opacity-0 backdrop-blur-sm transition-opacity group-hover:opacity-100">
                    {image.original_url && (
                      <button
                        onClick={() =>
                          onViewImage({
                            originalUrl: image.original_url,
                            generatedUrl: image.result_url || image.original_url!,
                            name: image.original_filename,
                          })
                        }
                        className="rounded-lg bg-purple-500/90 p-2 text-white transition-all hover:bg-purple-600"
                    ...
                    {image.result_url && (
                      <>
                        ...
                        <button
                          onClick={() => onDownloadImage(image.result_url!, image.original_filename)}
                          className="rounded-lg bg-white/10 p-2 text-white transition-all hover:bg-white/20"
                          title="Download"
                        >
                ...
                {/* Filename */}
                <div className="p-2">
                  <p className="truncate text-xs text-white/70" title={image.original_filename}>
                    {image.original_filename}
                  </p>
                  {image.error_message && (
                    <p className="mt-1 truncate text-xs text-red-400" title={image.error_message}>
                      {image.error_message}
                    </p>
                  )}
                </div>
