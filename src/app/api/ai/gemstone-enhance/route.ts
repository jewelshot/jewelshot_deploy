...
export async function POST(request: NextRequest) {
  try {
    ...
    if (creditError || !creditData) {
      logger.error('[GemstoneEnhance] Failed to check credits', {
        userId,
    ...
    if (typedCreditData.credits_remaining < 1) {
      logger.warn('[GemstoneEnhance] Insufficient credits', {
        userId,
      ...
      return NextResponse.json(
        {
          ...
          message: 'You need at least 1 credit to enhance gemstones.',
    ...
    try {
      ...
      try {
        ...
        const { error: deductError } = await (supabase as any).rpc(
          ...
          {
            p_user_id: userId,
            p_description: 'AI Gemstone Enhancement',
            p_metadata: {
              operation: 'gemstone-enhance',
            },
          }
        );

        if (deductError) {
          logger.error('[GemstoneEnhance] Failed to deduct credit', {
            userId,
            error: deductError.message,
          });
        } else {
          logger.info('[GemstoneEnhance] Credit deducted successfully', {
            userId,
          });
        }
      } catch (deductError) {
        logger.error('[GemstoneEnhance] Credit deduction error', {
          userId,
          deductError,
        });
      }
