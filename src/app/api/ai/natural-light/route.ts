...
export async function POST(request: NextRequest) {
  try {
    ...
    logger.info('[NaturalLight] User authenticated:', userId);
    ...
    if (creditError || !creditData) {
      logger.error('[NaturalLight] Failed to check credits', {
        userId,
    ...
    if (typedCreditData.credits_remaining < 1) {
      logger.warn('[NaturalLight] Insufficient credits', {
        userId,
      ...
      return NextResponse.json(
        {
          ...
          message: 'You need at least 1 credit to enhance natural light.',
    ...
    try {
      ...
      try {
        ...
        const { error: deductError } = await (supabase as any).rpc(
          ...
          {
            p_user_id: userId,
            p_description: 'AI Natural Light Enhancement',
            p_metadata: {
        ...
        if (deductError) {
          logger.error('[NaturalLight] Failed to deduct credit', {
            userId,
            error: deductError.message,
          });
        } else {
          logger.info('[NaturalLight] Credit deducted successfully', {
            userId,
          });
        }
      } catch (deductError) {
        logger.error('[NaturalLight] Credit deduction error', {
          userId,
          deductError,
        });
      }
